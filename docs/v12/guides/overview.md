...
2.  **Services:** The business logic layer validates your input and coordinates the work. For example, it ensures a new bookmark has a valid URL.
3.  **Repository:** This layer handles data storage. It abstracts the database, which in this case is a simple in-memory dictionary.
4.  **Models:** Plain Python classes (`Bookmark`, `Tag`) represent the data and are returned to the service layer. The final result is serialized to JSON and sent back to you.
