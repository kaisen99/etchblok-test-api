...
2.  **Route Defaults**: Some routes, such as `list_bookmarks` in `app/routes/bookmarks.py`, currently use a hardcoded default for pagination (`per_page=25`) in the `request.args.get` call rather than pulling the `PAGE_SIZE` from `app.config`.
